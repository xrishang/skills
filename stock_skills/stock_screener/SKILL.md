...
4. **写筛选报告** — 读 `candidates.json`,按 `assets/screen_report_template.md` 写 Markdown 报告 `全市场选股报告_{{YYYY-MM-DD}}.md`,列出 4 策略 Top N + 交叉验证
...
6. **生成 HTML + PDF** — 跑 `scripts/generate_pdf.py`,把筛选报告 + Top 3 个股报告合并,输出 `全市场选股报告_{{YYYY-MM-DD}}.html/.pdf`(同名,均保留;文件名含日期,对应"今天选了哪些票"这个问题)。Top 3 每只个股的报告按 `stock-investment-analysis` 的命名规则(`公司名(代码)投资分析报告.md/.html/.pdf`)生成
